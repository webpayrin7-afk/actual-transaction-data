...
export function RegionsPage() {
  ...
  const filteredSeoul = useMemo(() => {
    ...
    return SEOUL_REGIONS.filter((r) => scoreRegion(q, r) !== null);
  }, [regionQuery]);

  const filteredGyeonggi = useMemo(() => {
    const q = regionQuery.trim();
    if (!q) return GYEONGGI_REGIONS;
    return GYEONGGI_REGIONS.filter((r) => scoreRegion(q, r) !== null);
  }, [regionQuery]);
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 sm:p-6">
        <div className="flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <form onSubmit={onSubmit} className="w-full sm:max-w-xs">
