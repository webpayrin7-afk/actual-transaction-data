...
export type LabDonutSegment = {
  ...
  dimmed?: boolean;
  /** Short on-slice name (e.g. "33평"); the share % goes under it. Empty/omitted = % only. */
  label?: string;
};

/** Slices smaller than this carry no on-slice text (the list below still has them). */
...
/**
 * 구성비 도넛 (policy §12.7) — 조각 안에 이름·비율을 쓴다 (선 없음).
 * 항목 ≤4개·모든 조각 ≥5%일 때만 쓴다 — 그 밖은 정렬된 가로 막대(LabShareBars).
 * 도넛은 aria-hidden — 접근 가능한 수치는 함께 두는 목록이 담당한다.
 */
export function LabDonut({
  ...
  // Text sits mid-ring: between the outer edge and the hole.
