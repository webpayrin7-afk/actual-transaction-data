...
export function AptDetailPage({
  ...
  /** URL > 84㎡대 그룹/거래량 그룹 자동 > all (그룹 키 g:N) */
  ...
  const areaFiltered = useMemo(() => {
    ...
    // 그룹(floor)에 속한 모든 exact 전용면적 거래를 포함. 신고가 플래그는 exact areaKey 기준 유지.
    return data.items.filter((item) =>
      itemMatchesAreaGroup(Number(item.exclusiveArea), areaKey),
    );
  }, [data, areaKey]);
