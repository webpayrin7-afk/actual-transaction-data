...
export function LoanCalculator() {
  ...
  const valid =
    principalError == null && yearsError == null && rateError == null && ratePct != null && principalMan != null;
  ...
  function onApplyLimitMan(man: number) {
    const capped = Math.min(MAX_PRINCIPAL_MAN, Math.max(0, Math.round(man)));
    if (capped <= 0) return;
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 overflow-x-hidden px-4 py-6 sm:px-6">
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          대출금액, 금리, 기간과 상환방식을 입력해 월 상환액과 총이자를 계산하고
          금리 변화에 따른 부담 차이를 비교합니다.
        </p>
      </div>
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            >
              <input
                ...
                aria-describedby={
                  principalError ? `${principalId}-error` : `${principalId}-hint`
                }
                maxLength={11}
              />
            </Field>
                ...
                onChange={(e) => {
                  ...
                  const normalized =
                    parts.length > 2
                      ...
                      : next;
                  setRateRaw(normalized.slice(0, 6));
                }}
      ...
      <div aria-live="polite" className="flex flex-col gap-5">
        {current && comparison ? (
          <>
            <CurrentResult summary={current} lastDiffers={Boolean(lastDiffers)} />
            <RateDeltaSection
      ...
      <details className="rounded-2xl border border-slate-200 bg-white open:pb-4">
        <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/40">
          LTV·DSR·DTI 한도 참고
          <span className="ml-2 text-xs font-medium text-slate-400">선택</span>
        </summary>
        <div className="border-t border-slate-100 px-4 pt-4 sm:px-5">
          <LoanLimitCalculator
            defaultRate={rateRaw || "4.0"}
            onApplyLimitMan={onApplyLimitMan}
          />
        </div>
      </details>
    </div>
  ...
  return (
    <section className="rounded-2xl border border-teal-900/10 bg-gradient-to-br from-slate-900 via-teal-900 to-slate-800 p-5 text-white shadow-sm">
      ...
      <p className="mt-1 text-sm text-teal-100/75">
        {formatRatePct(summary.annualRatePct)} · {summary.years}년 ·{" "}
...
function RateDeltaSection({
  ...
  const step = formatRatePct(higher.ratePct - comparison.baseRatePct).replace("%", "%p");
        ...
        {method === "equal_payment" && delta.monthlyDeltaWon != null ? (
          <>
            ...
            {delta.annualDeltaWon != null ? (
              <DeltaRow label="연간 부담" value={formatSignedWon(delta.annualDeltaWon)} />
            ) : null}
            <DeltaRow label="총 이자" value={formatSignedWon(delta.totalInterestDeltaWon)} />
          </>
        ) : (
          <>
            <DeltaRow label="첫 달 납입액" value={formatSignedWon(delta.firstMonthDeltaWon)} />
            <DeltaRow label="마지막 달 납입액" value={formatSignedWon(delta.lastMonthDeltaWon)} />
            <DeltaRow label="총 이자" value={formatSignedWon(delta.totalInterestDeltaWon)} />
            <DeltaRow label="총 상환액" value={formatSignedWon(delta.totalPaymentDeltaWon)} />
          </>
        )}
      </dl>
    </section>
        ...
        {scenarios.map((scenario) => (
          ...
          >
            ...
            <dl className="mt-3 space-y-2 text-sm">
              {method === "equal_payment" && scenario.summary.monthlyPaymentWon != null ? (
                <CompactStat
                  label="월 상환액"
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
      ...
      <dl className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <CompactStat label="상환 횟수" value={`${summary.months.toLocaleString("ko-KR")}회`} />
        <CompactStat
          label="첫 해 납입액"
