  ...
  return (
    ...
    >
      ...
      <InfoSection title="금지행위">
        <InfoList
          items={[
            ...
            "로봇·크롤러·스크립트 등 자동화 수단으로 화면이나 데이터 API의 정보를 수집·저장하는 행위",
            "서비스의 정보·계산 결과를 인공지능(AI)·머신러닝 모델의 학습, 미세조정, 평가, 검색 증강(RAG) 등에 이용하는 행위",
            "서비스의 데이터·분석 결과를 모아 유사 서비스나 데이터베이스를 만드는 행위",
          ]}
        />
      </InfoSection>
      ...
      <InfoSection title="지식재산·콘텐츠">
        ...
        <p className="mt-2">
          서비스가 여러 원천 자료를 연결·가공해 만든 데이터베이스(단지·동·타입 연결, 3D 단지 모형, 보행 경로, 지표 계산
          결과 등)는 저작권법상 데이터베이스제작자의 권리로 보호됩니다. 운영자는 이 데이터베이스에 대해 텍스트·데이터
          마이닝(TDM) 권리를 유보하며, 인공지능 학습 목적의 수집을 허락하지 않습니다.
        </p>
      </InfoSection>
