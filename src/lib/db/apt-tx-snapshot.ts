...
export type RefreshResult = {
  ...
  failedLawds: string[];
  deferred: number;
  watermark: { from: number; to: number | null };
  stats: LawdBuildStats[];
};

/**
 * 일일 sync 끝(또는 스크립트)에서: 기준 번호 뒤에 바뀐 단지만 다시 만든다.
 * - 초기 적재 전(기준 번호 없음)이면 건너뛴다 — 요청 경로는 단지 번호로 거르므로 여전히 안전.
 * - 기준 번호는 목록을 읽기 전에 잡은 전역 번호로, 모든 lawd 가 에러 없이 끝났을 때만 올린다.
 *   그 뒤 변경·도중 경합(raced)은 다음 갱신 목록에 다시 나온다.
 * - 번호 표(tx_change_marks)는 seq 인덱스가 없어 표 전체(단지 수만큼)를 한 번 읽는다.
 */
export async function refreshAptTxSnapshots(
  db: Client,
  opts: { dryRun?: boolean; maxComplexes?: number; pauseMs?: number; log?: (msg: string) => void } = {},
  ...
  if (from == null) {
    return { skipped: "no watermark (초기 적재 전)", changed: 0, lawds: 0, failedLawds: [], deferred: 0, watermark: { from: -1, to: null }, stats: [] };
  }
  await assertTxChangeTriggers(db);
  const seqNow = await readGlobalChangeSeq(db);
  if (seqNow === from) {
    return { changed: 0, lawds: 0, failedLawds: [], deferred: 0, watermark: { from, to: from }, stats: [] };
  }
  const changed = await listChangedComplexesSince(db, from);
  const byLawd = new Map<string, string[]>();
  let taken = 0;
  let deferred = 0;
  for (const c of changed.sort((a, b) => a.seq - b.seq)) {
    if (taken >= maxComplexes) {
      deferred += 1;
      continue;
    }
    taken += 1;
  ...
  for (const [lawd, norms] of byLawd) {
    try {
      ...
      log(`[apt-tx-snapshot] refresh ${lawd} failed (요청 경로는 라이브 폴백): ${err instanceof Error ? err.message : String(err)}`);
      if (err instanceof AptTxSnapshotAnomaly && /멈춤/.test(err.message)) break;
    }
  }
  let to: number | null = null;
  if (!dryRun && failedLawds.length === 0 && deferred === 0 && stats.length === byLawd.size) {
    await writeAptTxSnapshotWatermark(db, seqNow);
    to = seqNow;
  }
  return { changed: changed.length, lawds: byLawd.size, failedLawds, deferred, watermark: { from, to }, stats };
}
