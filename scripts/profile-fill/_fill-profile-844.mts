...
const heldMissingCache = { household_count: 0, parking_total: 0, hubMissing: 0, kaptBassMissing: 0 };
...
function planOne(t: Target): Planned | null {
  ...
  const kHousehold = positiveOrNull(kapt?.bass?.kaptdaCnt);
  ...
  // A failed fetch leaves no cache. Without both sides, the hub/K-apt cross-check
  // cannot run, so household/parking wait for a refetch instead of being written one-sided.
  const hubMissing = Boolean(t.pnu) && (!recap || (recap.total === 0 && !title));
  const bassMissing = Boolean(t.kapt) && !kapt?.bassOk;
  const dtlMissing = Boolean(t.kapt) && !kapt?.dtlOk;
  const holdHh = t.profile.household_count == null && (hubMissing || bassMissing);
  const holdPark = t.profile.parking_total == null && (hubMissing || dtlMissing);
  if (holdHh) heldMissingCache.household_count += 1;
  if (holdPark) heldMissingCache.parking_total += 1;
  if (hubMissing) heldMissingCache.hubMissing += 1;
  if (bassMissing) heldMissingCache.kaptBassMissing += 1;
  const hh = holdHh ? { value: null, conflict: false } : pickAgreed(hub?.household ?? null, kHousehold);
  const park = holdPark
    ? { value: null, conflict: false }
    : pickAgreed(hub?.parking != null ? Math.trunc(hub.parking) : null, kPark != null ? Math.trunc(kPark) : null);
...
async function planPhase(client: Client, apply: boolean) {
  ...
  mkdirSync(OUT, { recursive: true });
  Object.assign(totals, { heldMissingCache });
