/**
 * Gyeonggi historical management-fee closeout.
 * Freezes every never-attempted exact Gyeonggi identity into one cohort.
 * Acquires in segments of 40. Identity guard: frozen KAPT must still match live link.
 * No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-gyeonggi-final.mts
 */
...
const REPORT_PATH = resolve(DIR, "gyeonggi-final-dryrun.json");
const STATE_PATH = resolve(DIR, "gyeonggi-final-segment-state.json");
const IDENTITY_GUARD_PATH = resolve(DIR, "gyeonggi-final-identity-guard.json");
const SIDO_CODE = "41";
const SIDO_NAME = "경기도";
...
function loadState(): void {
  const state = existsSync(STATE_PATH)
    ...
    : emptyState();
  if (state.version !== 1 || state.wave !== "gyeonggi" || state.segment_size !== SEGMENT_SIZE) {
    throw new Error("gyeonggi segment state mismatch");
  }
...
async function selectAndFreezeCohort(): Promise<CohortRow[]> {
  ...
  const db = openReadOnlyClient();
  const masterSql = "SELECT COUNT(*) AS n FROM apt_complex_master WHERE sido_code = '41'";
  assertReadOnlySql(masterSql);
  const masterResult = await db.execute(masterSql);
  const gyeonggiMaster = Number(masterResult.rows[0]?.n);
  ...
  if (multiCode !== 0) throw new Error(`gyeonggi identity duplicates multi=${multiCode}`);
  const gyeonggiShared = new Set<string>();
  for (const row of rows) {
    if (row.sido_code !== SIDO_CODE) continue;
    for (const code of row.kapt_codes) if (shared.has(code)) gyeonggiShared.add(code);
  }
  if (gyeonggiShared.size !== 0) throw new Error(`gyeonggi shared kapt ${gyeonggiShared.size}`);
  ...
  const body = {
    ...
    frozen: true,
    selection_rule:
      "Gyeonggi historical closeout. All confirmed exact KAPT identities (one complex, one A######## code, not shared) that are not already fee-covered. Rank band A (tx_12m desc), B (supply_area_sqm>0), C (tx_30d desc), D, then complex_id asc. No arbitrary Wave subsets. No membership cap.",
    segment_size: SEGMENT_SIZE,
    gyeonggi_master: gyeonggiMaster,
  ...
  writeSync(
    1,
    `cohort master=${gyeonggiMaster} confirmed=${confirmedExact} covered=${alreadyCovered} never_attempted=${ready.length} selected=${selected.length} baseline=${baseline.rows}/${baseline.complexes}\n`,
  );
  return selected;
}
...
async function writeFinalReport(cohort: readonly CohortRow[], calls: readonly LiveCall[]): Promise<void> {
  ...
  const report = {
    ...
    complexes_by_sido: { "41": cohort.length },
    checkpoint_records: store.list().length,
    no_published_month: counts.NO_PUBLISHED_MONTH,
    no_published_month_by_sido: { "41": counts.NO_PUBLISHED_MONTH },
  ...
  writeSync(1, "GYEONGGI_COHORT_TERMINAL\n");
}
...
async function main(): Promise<void> {
  ...
  while (snapshotClassifications().unfinished.length > 0) {
    if (Date.now() - processStarted > PROCESS_BUDGET_MS && stats.segments_completed > 0) {
      ...
      writeSync(
        1,
        `GYEONGGI_SEGMENT_PAUSE segments=${stats.segments_completed} terminal=${Object.keys(snap.classifications).length} unfinished=${snap.unfinished.length} api=${stats.api_calls} retries=${stats.retries} 429=${stats.http_429} 5xx=${stats.http_5xx} timeout=${stats.timeouts}\n`,
      );
      return;
    }
