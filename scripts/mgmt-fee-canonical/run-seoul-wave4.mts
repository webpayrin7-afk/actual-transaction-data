/**
 * Seoul fee wave 4 dry-run.
 * Freezes up to 200 never-attempted exact Seoul identities after waves 1-3.
 * Includes profile-promoted exact links. Acquires in segments of 40. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-seoul-wave4.mts
 */
...
const REPORT_PATH = resolve(DIR, "seoul-wave4-dryrun.json");
const STATE_PATH = resolve(DIR, "seoul-wave4-segment-state.json");
const SIDO_CODE = "11";
...
async function selectAndFreezeCohort(): Promise<CohortRow[]> {
  ...
  const priorIds = new Set<string>([...wave1Ids, ...wave2Ids, ...wave3Ids]);
  if (priorIds.size !== wave1Ids.size + wave2Ids.size + wave3Ids.size) {
  ...
  if (selected.some((row) => priorIds.has(row.complex_id))) throw new Error("wave 4 overlaps an earlier seoul cohort");
  ...
  const body = {
    ...
    frozen: true,
    selection_rule:
      "Current confirmed Seoul exact KAPT identities (one complex, one A######## code, not shared), including EXACT_MULTI_SIGNAL profile promotions. Exclude wave 1, wave 2, and wave 3 cohorts regardless of outcome, plus already fee-covered complexes. Rank band A (tx_12m desc), B (supply_area_sqm>0), C (tx_30d desc), D, then complex_id asc. No random sampling. Cap 200.",
    segment_size: SEGMENT_SIZE,
...
async function writeFinalReport(cohort: readonly CohortRow[], calls: readonly LiveCall[]): Promise<void> {
  ...
  writeSync(1, "WAVE4_COHORT_TERMINAL\n");
}
...
async function main(): Promise<void> {
  ...
  const wave2Ids = loadCohortIds(WAVE2_COHORT_PATH, 200);
  const wave3Ids = loadCohortIds(WAVE3_COHORT_PATH, 200);
  if (cohort.some((row) => wave1Ids.has(row.complex_id) || wave2Ids.has(row.complex_id) || wave3Ids.has(row.complex_id))) {
    throw new Error("frozen cohort overlaps wave 1, 2, or 3");
  }
  if (process.argv.includes("--freeze-only")) {
    writeSync(1, "WAVE4_FROZEN\n");
    return;
  }
  ...
  for (const id of checkpointIds) {
    if (!cohortIds.has(id)) throw new Error(`checkpoint complex outside wave 4 cohort ${id}`);
  }
  ...
  while (snapshotClassifications().unfinished.length > 0) {
    if (Date.now() - processStarted > PROCESS_BUDGET_MS && stats.segments_completed > 0) {
      ...
      writeSync(
        1,
        `WAVE4_SEGMENT_PAUSE segments=${stats.segments_completed} terminal=${Object.keys(snap.classifications).length} unfinished=${snap.unfinished.length} api=${stats.api_calls} retries=${stats.retries} 429=${stats.http_429} 5xx=${stats.http_5xx} timeout=${stats.timeouts}\n`,
      );
      return;
    }
