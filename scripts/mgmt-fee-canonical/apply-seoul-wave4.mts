/**
 * Production INSERT of the verified Seoul wave-4 COMPLETE months.
 * Rebuilds the 176 rows from the committed Seoul wave-4 checkpoint. Does not call the fee API.
 * A second --commit of the same artifact is a no-op when every stored row already matches.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave4.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave4.mts --commit
 */
...
const CHECKPOINT_SHA256 = "b1e5fdeb9340456fca133247fa155b3dd4afef18274d3beaeb33164922a12239";
const DRYRUN_SHA256 = "7ca9e3356a762f54924c7b8f5704b516bfb3b21497d4cdf37c35f6fa1b42c6f8";
const COHORT_SHA256 = "b73d7fdfd642a5d713d72e5b19ca6e64a621464419c3e7e09f508f99bee8a45d";
const EXPECTED_BEFORE_ROWS = 1404;
const EXPECTED_BEFORE_COMPLEXES = 1291;
const EXPECTED_AFTER_ROWS = 1580;
const EXPECTED_AFTER_COMPLEXES = 1467;
...
const CHECKPOINT_PATH = resolve(DIR, "seoul-wave4-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "seoul-wave4-dryrun.json");
const COHORT_PATH = resolve(DIR, "seoul-wave4-cohort.json");
const RESULT_PATH = resolve(DIR, "seoul-wave4-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (!cohort.frozen || cohort.selected !== 200 || cohort.complexes.length !== 200) failures.push("frozen cohort");
  ...
  if (artifact.api_calls !== 5352 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 176 || artifact.no_published_month !== 24) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 176 || artifact.totals.would_insert !== 176 || artifact.totals.would_update !== 0) {
    failures.push("artifact totals");
  }
  const artifactSeoul = artifact.by_sido.find((row) => row.sido_code === "11");
  if (
    !artifactSeoul ||
    artifactSeoul.targets !== 176 ||
    artifactSeoul.COMPLETE !== 176 ||
    artifactSeoul.would_insert !== 176 ||
    artifactSeoul.would_update !== 0 ||
    artifactSeoul.PARTIAL !== 0 ||
    artifactSeoul.MISSING !== 0 ||
    artifactSeoul.FAILED !== 0
  ) {
    failures.push("artifact seoul");
  }
  ...
  if (records.length !== 5352) failures.push(`checkpoint records ${records.length}`);
  ...
  if (checkpointIds.size !== 200 || cohort.complexes.some((row) => !checkpointIds.has(row.complex_id))) {
  ...
  if (plans.length !== 176 || unpublished.length !== 24) {
  ...
  if (plans.length > 0) {
    ...
    for (const id of unpublished) {
      ...
      if (code !== "11") failures.push(`unpublished sido ${code}`);
    }
  }

  const seoul = plans.filter((plan) => plan.sido_code === "11");
  if (plans.length === 176 && seoul.length !== 176) {
    failures.push(`sido split seoul ${seoul.length}`);
  }
  ...
  const samplePlans = seoul.slice(0, 10);
  ...
  let insertedSeoul = 0;
  try {
    await applySido(db, seoul, updatedAt, originalKeys, originalHash);
    insertedSeoul = seoul.length;
  } catch (error) {
    ...
    writeResult({
      status: "ROLLED_BACK",
      reason: message,
      inserted_seoul: insertedSeoul,
      rows_now: after.length,
      rollback_executed: true,
      production_write: insertedSeoul > 0,
      api_calls: 0,
    });
    console.log(JSON.stringify({ status: "ROLLED_BACK", reason: message, insertedSeoul, api_calls: 0 }));
    process.exit(3);
  }
  ...
  if (insertedSeoul !== 176) post.push("insert counts");
