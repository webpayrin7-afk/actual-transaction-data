/**
 * Busan + Gyeongnam fee wave 6 dry-run.
 * Skips the wave 1 through wave 5 cohorts. One published month. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-busan-gyeongnam-wave6.mts
 */
...
const WAVE4_CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave4-op-checkpoint.json");
const WAVE4_CHECKPOINT_SHA256 = "112caac61a5670e41c1a0c1d3513b14eb7f93f99107a0756d1f1b28a71afbda1";
const WAVE5_CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave5-op-checkpoint.json");
const WAVE5_CHECKPOINT_SHA256 = "f8e305c603ab8140439888d10a1e3b054c272526d0d8350b837367789bb31bbc";
const CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave6-op-checkpoint.json");
const REPORT_PATH = resolve(DIR, "busan-gyeongnam-wave6-dryrun.json");
...
function priorComplexIds(): Set<string> {
  ...
  const wave4 = pinnedComplexIds(WAVE4_CHECKPOINT_PATH, WAVE4_CHECKPOINT_SHA256, "wave 4", 200);
  const wave5 = pinnedComplexIds(WAVE5_CHECKPOINT_PATH, WAVE5_CHECKPOINT_SHA256, "wave 5", 200);
  for (const [label, ids] of [["wave 2", wave2], ["wave 3", wave3], ["wave 4", wave4], ["wave 5", wave5]] as const) {
    for (const id of ids) {
      if (prior.has(id)) throw new Error(`${label} overlaps an earlier cohort`);
      prior.add(id);
    }
  }
  if (prior.size !== 750) throw new Error(`prior cohort ${prior.size}`);
  return prior;
}
