/**
 * Production INSERT of the verified Gyeonggi final COMPLETE months.
 * Rebuilds the 1080 rows from the committed Gyeonggi final checkpoint. Does not call the fee API.
 * A second --commit of the same artifact is a no-op when every stored row already matches.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-gyeonggi-final.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-gyeonggi-final.mts --commit
 */
...
const CHECKPOINT_SHA256 = "f36830429f5bcd0b0ef532a08b0022ed74eee59934e27e0a40c029618018bcde";
const DRYRUN_SHA256 = "c510c7919dd0f1aed10130bea0560cb99e9d21b4d0555104a2c43969ae6277f6";
const COHORT_SHA256 = "b217a2d8032fcd277152b6656fb04025a1af6196acdf2add923d374ccc1db714";
const EXPECTED_BEFORE_ROWS = 1843;
const EXPECTED_BEFORE_COMPLEXES = 1730;
const EXPECTED_AFTER_ROWS = 2923;
const EXPECTED_AFTER_COMPLEXES = 2810;
...
const CHECKPOINT_PATH = resolve(DIR, "gyeonggi-final-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "gyeonggi-final-dryrun.json");
const COHORT_PATH = resolve(DIR, "gyeonggi-final-cohort.json");
const RESULT_PATH = resolve(DIR, "gyeonggi-final-apply-result.json");
...
function rebuildPlans(store: OpCheckpointStore, cohortIds: readonly string[]): { plans: InsertPlan[]; unpublished: string[] } {
...
async function main(): Promise<void> {
  ...
  if (sha256File(COHORT_PATH) !== COHORT_SHA256) failures.push("cohort hash");
  ...
  if (!cohort.frozen || cohort.selected !== 1150 || cohort.complexes.length !== 1150) failures.push("frozen cohort");
  ...
  if (artifact.api_calls !== 32610 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 1080 || artifact.no_published_month !== 70) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 1080 || artifact.totals.would_insert !== 1080 || artifact.totals.would_update !== 0) {
    failures.push("artifact totals");
  }
  const artifactGyeonggi = artifact.by_sido.find((row) => row.sido_code === "41");
  if (
    !artifactGyeonggi ||
    artifactGyeonggi.targets !== 1080 ||
    artifactGyeonggi.COMPLETE !== 1080 ||
    artifactGyeonggi.would_insert !== 1080 ||
    artifactGyeonggi.would_update !== 0 ||
    artifactGyeonggi.PARTIAL !== 0 ||
    artifactGyeonggi.MISSING !== 0 ||
    artifactGyeonggi.FAILED !== 0
  ) {
    failures.push("artifact gyeonggi");
  }
  ...
  if (records.length !== 32610) failures.push(`checkpoint records ${records.length}`);
  ...
  const checkpointIds = new Set(records.map((record) => record.complex_id));
  if (checkpointIds.size !== 1150 || cohort.complexes.some((row) => !checkpointIds.has(row.complex_id))) {
    failures.push("checkpoint cohort mismatch");
  }

  let plans: InsertPlan[] = [];
  let unpublished: string[] = [];
  try {
    const rebuilt = rebuildPlans(store, cohort.complexes.map((row) => row.complex_id));
    plans = rebuilt.plans;
    unpublished = rebuilt.unpublished;
  } catch (error) {
    failures.push(error instanceof Error ? error.message : "rebuild failed");
  }
  if (plans.length !== 1080 || unpublished.length !== 70) {
  ...
  if (plans.length > 0) {
    ...
    for (const id of unpublished) {
      ...
      if (code !== "41") failures.push(`unpublished sido ${code}`);
    }
  }

  const gyeonggi = plans.filter((plan) => plan.sido_code === "41");
  if (plans.length === 1080 && gyeonggi.length !== 1080) {
    failures.push(`sido split gyeonggi ${gyeonggi.length}`);
  }
  ...
  const samplePlans = gyeonggi.slice(0, 10);
  ...
  let insertedGyeonggi = 0;
  try {
    await applySido(db, gyeonggi, updatedAt, originalKeys, originalHash);
    insertedGyeonggi = gyeonggi.length;
  } catch (error) {
    ...
    writeResult({
      status: "ROLLED_BACK",
      reason: message,
      inserted_gyeonggi: insertedGyeonggi,
      rows_now: after.length,
      rollback_executed: true,
      production_write: insertedGyeonggi > 0,
      api_calls: 0,
    });
    console.log(JSON.stringify({ status: "ROLLED_BACK", reason: message, insertedGyeonggi, api_calls: 0 }));
    process.exit(3);
  }
  ...
  if (insertedGyeonggi !== 1080) post.push("insert counts");
