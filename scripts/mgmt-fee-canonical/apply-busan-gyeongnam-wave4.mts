/**
 * Production INSERT of the verified Busan/Gyeongnam wave-4 COMPLETE months.
 * Rebuilds the 175 rows from the committed wave-4 checkpoint. Does not call the fee API.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave4.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave4.mts --commit
 */
...
const CHECKPOINT_SHA256 = "112caac61a5670e41c1a0c1d3513b14eb7f93f99107a0756d1f1b28a71afbda1";
const DRYRUN_SHA256 = "e9d4a1ba3ebd39f1002e615c9eb4b07007f4fc2883a7145fe0212679a8edb500";
const EXPECTED_BEFORE_ROWS = 432;
const EXPECTED_BEFORE_COMPLEXES = 319;
const EXPECTED_AFTER_ROWS = 607;
const EXPECTED_AFTER_COMPLEXES = 494;
...
const DRYRUN_PATH = resolve(DIR, "busan-gyeongnam-wave4-dryrun.json");
const RESULT_PATH = resolve(DIR, "busan-gyeongnam-wave4-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 5325 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 175 || artifact.no_published_month !== 25) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 175 || artifact.totals.would_insert !== 175 || artifact.totals.would_update !== 0) {
  ...
  if (
    !artifactBusan ||
    artifactBusan.targets !== 87 ||
    artifactBusan.COMPLETE !== 87 ||
    artifactBusan.would_insert !== 87 ||
  ...
  if (
    !artifactGyeongnam ||
    artifactGyeongnam.targets !== 88 ||
    artifactGyeongnam.COMPLETE !== 88 ||
    artifactGyeongnam.would_insert !== 88 ||
  ...
  if (records.length !== 5325) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 175 || unpublished.length !== 25) {
  ...
  if (plans.length === 175 && (busan.length !== 87 || gyeongnam.length !== 88)) {
  ...
  if (insertedBusan !== 87 || insertedGyeongnam !== 88) post.push("insert counts");
