/**
 * Production INSERT of the verified Busan/Gyeongnam wave-5 COMPLETE months.
 * Rebuilds the 174 rows from the committed wave-5 checkpoint. Does not call the fee API.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave5.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave5.mts --commit
 */
...
const CHECKPOINT_SHA256 = "f8e305c603ab8140439888d10a1e3b054c272526d0d8350b837367789bb31bbc";
const DRYRUN_SHA256 = "784d512892d49c7540ce6a7d49bf40570b273071f8f43cb1f1aca326c8338a87";
const EXPECTED_BEFORE_ROWS = 607;
const EXPECTED_BEFORE_COMPLEXES = 494;
const EXPECTED_AFTER_ROWS = 781;
const EXPECTED_AFTER_COMPLEXES = 668;
...
const CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave5-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "busan-gyeongnam-wave5-dryrun.json");
const RESULT_PATH = resolve(DIR, "busan-gyeongnam-wave5-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 5298 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 174 || artifact.no_published_month !== 26) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 174 || artifact.totals.would_insert !== 174 || artifact.totals.would_update !== 0) {
  ...
  if (
    !artifactBusan ||
    artifactBusan.targets !== 85 ||
    artifactBusan.COMPLETE !== 85 ||
    artifactBusan.would_insert !== 85 ||
  ...
  if (records.length !== 5298) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 174 || unpublished.length !== 26) {
  ...
  if (plans.length === 174 && (busan.length !== 85 || gyeongnam.length !== 89)) {
  ...
  if (insertedBusan !== 85 || insertedGyeongnam !== 89) post.push("insert counts");
