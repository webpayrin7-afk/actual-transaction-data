/**
 * Seoul fee wave 3 dry-run.
 * Freezes the next exact-safe READY Seoul complexes after waves 1 and 2.
 * Acquires in segments of 40. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-seoul-wave3.mts
 */
...
const REPORT_PATH = resolve(DIR, "seoul-wave3-dryrun.json");
const STATE_PATH = resolve(DIR, "seoul-wave3-segment-state.json");
const SIDO_CODE = "11";
...
function loadState(): void {
  ...
  if (state.version !== 1 || state.wave !== 3 || state.segment_size !== SEGMENT_SIZE) {
    throw new Error("wave 3 segment state mismatch");
  }
...
async function selectAndFreezeCohort(): Promise<CohortRow[]> {
  ...
  for (const id of wave1Ids) {
    if (wave2Ids.has(id)) throw new Error("wave 1 overlaps wave 2");
  }
  const db = openReadOnlyClient();
  ...
  for (const row of linkRows.rows) {
    ...
    if (tier === "EXACT_SAFE" && KAPT_CODE_RE.test(kapt)) exactSafe.set(complexId, kapt);
  }
  const rows = await readNationalComplexes(db);
  const shared = sharedKaptCodes(rows);
  let confirmedSeoul = 0;
  let alreadyCovered = 0;
  let excludedWave1Ready = 0;
  let excludedWave2Ready = 0;
  ...
  for (const row of rows) {
    ...
    if (wave1Ids.has(row.complex_id)) {
      excludedWave1Ready += 1;
      continue;
    }
    if (wave2Ids.has(row.complex_id)) {
      excludedWave2Ready += 1;
      continue;
    }
    ready.push({
  ...
  const selected = ranked.slice(0, Math.min(TOTAL_CAP, ranked.length));
  if (selected.some((row) => wave1Ids.has(row.complex_id) || wave2Ids.has(row.complex_id))) {
    throw new Error("wave 3 overlaps an earlier seoul cohort");
  }
  ...
  const body = {
    ...
    frozen: true,
    selection_rule:
      "Confirmed Seoul KAPT only. Exclude wave 1 cohort, wave 2 cohort, fee-covered, non-EXACT_SAFE, unsafe, shared, or ambiguous. Rank band A (tx_12m desc), B (supply_area_sqm>0), C (tx_30d desc), D, then complex_id asc. No random sampling. Cap 200.",
    segment_size: SEGMENT_SIZE,
    confirmed_seoul: confirmedSeoul,
    already_fee_covered: alreadyCovered,
    remaining_eligible: ready.length,
    excluded_wave1_cohort: wave1Ids.size,
    excluded_wave2_cohort: wave2Ids.size,
    excluded_wave1_still_ready: excludedWave1Ready,
    excluded_wave2_still_ready: excludedWave2Ready,
    excluded_already_covered: alreadyCovered,
  ...
  writeSync(
    1,
    `cohort confirmed=${confirmedSeoul} covered=${alreadyCovered} eligible=${ready.length} selected=${selected.length} excluded_wave1_ready=${excludedWave1Ready} excluded_wave2_ready=${excludedWave2Ready} excluded_unsafe=${excludedUnsafe} baseline=${baseline.rows}/${baseline.complexes}\n`,
  );
  return selected;
}
...
async function writeFinalReport(cohort: readonly CohortRow[], calls: readonly LiveCall[]): Promise<void> {
  ...
  if (confirmTargets.length > 0) {
    const confirm = await runExpansionDryRun({
      ...
      calls,
      store,
      prior: confirmTargets.map((target) => {
  ...
  writeSync(1, "WAVE3_COHORT_TERMINAL\n");
}
...
async function main(): Promise<void> {
  ...
  const wave1Ids = loadCohortIds(WAVE1_COHORT_PATH, 100);
  const wave2Ids = loadCohortIds(WAVE2_COHORT_PATH, 200);
  if (cohort.some((row) => wave1Ids.has(row.complex_id) || wave2Ids.has(row.complex_id))) {
    throw new Error("frozen cohort overlaps wave 1 or 2");
  }
  ...
  for (const id of checkpointIds) {
    if (!cohortIds.has(id)) throw new Error(`checkpoint complex outside wave 3 cohort ${id}`);
  }
  ...
  while (snapshotClassifications().unfinished.length > 0) {
    if (Date.now() - processStarted > PROCESS_BUDGET_MS && stats.segments_completed > 0) {
      ...
      writeSync(
        1,
        `WAVE3_SEGMENT_PAUSE segments=${stats.segments_completed} terminal=${Object.keys(snap.classifications).length} unfinished=${snap.unfinished.length} api=${stats.api_calls} retries=${stats.retries} 429=${stats.http_429} 5xx=${stats.http_5xx} timeout=${stats.timeouts}\n`,
      );
      return;
    }
