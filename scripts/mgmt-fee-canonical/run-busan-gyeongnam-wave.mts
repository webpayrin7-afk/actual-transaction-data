/**
 * Busan + Gyeongnam fee dry-run.
 * Explicit KAPT links only. One published month. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-busan-gyeongnam-wave.mts
 */
...
const REPORT_PATH = resolve(DIR, "busan-gyeongnam-wave-dryrun.json");
...
async function main(): Promise<void> {
  ...
  const scoped = rows.filter((row) => row.sido_code === "26" || row.sido_code === "48");
  ...
  const mappingMismatch = cohort.filter(
    (row) => !/^A\d{8}$/.test(row.kapt_code) || shared.has(row.kapt_code) || row.state !== "READY",
  ).length;
  if (cohort.length > 50 || mappingMismatch !== 0) {
    throw new Error(`cohort guard complexes=${cohort.length} mismatch=${mappingMismatch}`);
  }
  ...
  if (targets.length > 50) throw new Error("target-month cap");
  ...
  writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
  writeSync(1, `${JSON.stringify({ stopped: dry.stopped, api_calls: apiCalls, targets: targets.length, totals })}\n`);
