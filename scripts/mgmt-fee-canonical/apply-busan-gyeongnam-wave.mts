/**
 * Production INSERT of the verified Busan/Gyeongnam wave-1 COMPLETE months.
 * Rebuilds the 41 rows from the committed checkpoint. Does not call the fee API.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave.mts --commit
 */
...
const CHECKPOINT_SHA256 = "24b446009549bf7a95aac5bf0e20f3923a5fc3dfa2b0bde638a574e697e609d8";
const DRYRUN_SHA256 = "9cf4d08b199d3680bbf3d4c02448f6db83e6fdb07380d4c8dda2ca913667b0dc";
const EXPECTED_BEFORE_ROWS = 124;
const EXPECTED_BEFORE_COMPLEXES = 11;
const EXPECTED_AFTER_ROWS = 165;
const EXPECTED_AFTER_COMPLEXES = 52;
...
const CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "busan-gyeongnam-wave-dryrun.json");
const RESULT_PATH = resolve(DIR, "busan-gyeongnam-wave-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 1257 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 41 || artifact.unpublished_complexes !== 9) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 41 || artifact.totals.would_insert !== 41 || artifact.totals.would_update !== 0) {
  ...
  if (records.length !== 1257) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 41 || unpublished.length !== 9) {
  ...
  if (plans.length === 41 && (busan.length !== 20 || gyeongnam.length !== 21)) {
  ...
  if (insertedBusan !== 20 || insertedGyeongnam !== 21) post.push("insert counts");
  for (const plan of plans) {
    const stored = afterIndex.get(rowKey(plan.complex_id, plan.period_yyyymm));
