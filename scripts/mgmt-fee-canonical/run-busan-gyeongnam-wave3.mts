/**
 * Busan + Gyeongnam fee wave 3 dry-run.
 * Skips the wave 1 and wave 2 cohorts. One published month. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-busan-gyeongnam-wave3.mts
 */
...
const REPORT_PATH = resolve(DIR, "busan-gyeongnam-wave3-dryrun.json");
...
function priorComplexIds(): Set<string> {
  const wave1 = pinnedComplexIds(WAVE1_CHECKPOINT_PATH, WAVE1_CHECKPOINT_SHA256, "wave 1", 50);
  const wave2 = pinnedComplexIds(WAVE2_CHECKPOINT_PATH, WAVE2_CHECKPOINT_SHA256, "wave 2", 100);
  for (const id of wave2) {
    if (wave1.has(id)) throw new Error("wave 2 overlaps wave 1");
    wave1.add(id);
  }
  if (wave1.size !== 150) throw new Error(`prior cohort ${wave1.size}`);
  return wave1;
}
...
async function main(): Promise<void> {
  ...
  if (resumedOverlap !== 0) throw new Error("wave 3 checkpoint contains an earlier cohort");
  ...
  const report = {
    ...
    stopped: dry.stopped,
    selected_sidos: [...SIDO_CODES],
    excluded_wave1_wave2: prior.size,
