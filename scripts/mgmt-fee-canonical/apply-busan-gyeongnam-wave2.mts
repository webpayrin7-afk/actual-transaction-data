/**
 * Production INSERT of the verified Busan/Gyeongnam wave-2 COMPLETE months.
 * Rebuilds the 88 rows from the committed wave-2 checkpoint. Does not call the fee API.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave2.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave2.mts --commit
 */
...
const CHECKPOINT_SHA256 = "7a149440cd4b3afa97fd19af93f4db6f9b6fe63d92061a88bdacec98b199c755";
const DRYRUN_SHA256 = "b0e9d9d70fd7f8e66513ceffdfa570751616ddd9018b08c595be7a7e88ef1093";
const EXPECTED_BEFORE_ROWS = 165;
const EXPECTED_BEFORE_COMPLEXES = 52;
const EXPECTED_AFTER_ROWS = 253;
const EXPECTED_AFTER_COMPLEXES = 140;
...
const CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave2-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "busan-gyeongnam-wave2-dryrun.json");
const RESULT_PATH = resolve(DIR, "busan-gyeongnam-wave2-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 2676 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 88 || artifact.no_published_month !== 12) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 88 || artifact.totals.would_insert !== 88 || artifact.totals.would_update !== 0) {
  ...
  if (
    !artifactBusan ||
    artifactBusan.targets !== 40 ||
    artifactBusan.COMPLETE !== 40 ||
    artifactBusan.would_insert !== 40 ||
  ...
  if (
    !artifactGyeongnam ||
    artifactGyeongnam.targets !== 48 ||
    artifactGyeongnam.COMPLETE !== 48 ||
    artifactGyeongnam.would_insert !== 48 ||
  ...
  if (records.length !== 2676) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 88 || unpublished.length !== 12) {
  ...
  if (plans.length === 88 && (busan.length !== 40 || gyeongnam.length !== 48)) {
  ...
  if (insertedBusan !== 40 || insertedGyeongnam !== 48) post.push("insert counts");
