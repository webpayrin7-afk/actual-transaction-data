/**
 * Busan + Gyeongnam fee wave 5 dry-run.
 * Skips the wave 1 through wave 4 cohorts. One published month. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-busan-gyeongnam-wave5.mts
 */
...
const REPORT_PATH = resolve(DIR, "busan-gyeongnam-wave5-dryrun.json");
...
function priorComplexIds(): Set<string> {
  ...
  for (const [label, ids] of [["wave 2", wave2], ["wave 3", wave3], ["wave 4", wave4]] as const) {
    for (const id of ids) {
      if (prior.has(id)) throw new Error(`${label} overlaps an earlier cohort`);
      prior.add(id);
    }
  }
  if (prior.size !== 550) throw new Error(`prior cohort ${prior.size}`);
  return prior;
}
...
async function main(): Promise<void> {
  ...
  if (resumedOverlap !== 0) throw new Error("wave 5 checkpoint contains an earlier cohort");
