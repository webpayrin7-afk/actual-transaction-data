/**
 * Busan + Gyeongnam fee wave 2 dry-run.
 * Skips the wave 1 cohort. One published month. No Production write.
 *
 *   npx tsx scripts/mgmt-fee-canonical/run-busan-gyeongnam-wave2.mts
 */
...
const REPORT_PATH = resolve(DIR, "busan-gyeongnam-wave2-dryrun.json");
...
function wave1ComplexIds(): Set<string> {
  const bytes = readFileSync(WAVE1_CHECKPOINT_PATH);
  const hash = createHash("sha256").update(bytes).digest("hex");
  if (hash !== WAVE1_CHECKPOINT_SHA256) throw new Error("wave 1 checkpoint hash mismatch");
  const ids = new Set(OpCheckpointStore.parse(bytes.toString("utf8")).list().map((record) => record.complex_id));
  if (ids.size !== 50) throw new Error(`wave 1 cohort ${ids.size}`);
  return ids;
}
...
async function main(): Promise<void> {
  ...
  const wave1 = wave1ComplexIds();
  const db = openReadOnlyClient();
  const rows = await readNationalComplexes(db);
  const shared = sharedKaptCodes(rows);
  const scoped = rows.filter((row) => (row.sido_code === "26" || row.sido_code === "48") && !wave1.has(row.complex_id));
  ...
  const mappingMismatch = cohort.filter(
    (row) => !/^A\d{8}$/.test(row.kapt_code) || shared.has(row.kapt_code) || row.state !== "READY" || wave1.has(row.complex_id),
  ).length;
  ...
  const resumedOverlap = store.list().filter((record) => wave1.has(record.complex_id)).length;
  if (resumedOverlap !== 0) throw new Error("wave 2 checkpoint contains wave 1 complexes");
