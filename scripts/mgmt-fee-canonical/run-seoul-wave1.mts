/**
 * Seoul fee wave 1 dry-run.
 * Freezes 100 READY Seoul complexes by product priority. One published month.
...
const REPORT_PATH = resolve(DIR, "seoul-wave1-dryrun.json");
const SIDO_CODE = "11";
...
async function selectAndFreezeCohort(): Promise<CohortRow[]> {
  ...
  const ready = rows
    .filter((row) => row.sido_code === SIDO_CODE && classifyComplex(row, shared) === "READY")
    .map((row) => ({
  ...
  if (ready.length === 0) throw new Error("no Seoul READY unloaded complexes");
...
async function main(): Promise<void> {
  ...
  if (checkpointIds.size > 0) {
    if (
      ...
      throw new Error(`seoul wave1 resume cohort mismatch checkpoint=${checkpointIds.size} planned=${cohort.length}`);
    }
  }
