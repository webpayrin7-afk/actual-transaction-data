/**
 * Production INSERT of the verified Seoul wave-3 COMPLETE months.
 * Rebuilds the 169 rows from the committed Seoul wave-3 checkpoint. Does not call the fee API.
 * A second --commit of the same artifact is a no-op when every stored row already matches.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave3.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave3.mts --commit
 */
...
const CHECKPOINT_SHA256 = "09d5a12ca5fb070666218200fe3dc84ee33132c43f04d4be6d73b1840bafa6ed";
const DRYRUN_SHA256 = "d3d4d7257effe5219280cdc23554c0bfb5b4e78c783b4022d8962400d434f936";
const COHORT_SHA256 = "3c2aef0309c140b01377e7e563a5c338c4e2a980e02000b4b667d8e75c9bfe36";
const EXPECTED_BEFORE_ROWS = 1235;
const EXPECTED_BEFORE_COMPLEXES = 1122;
const EXPECTED_AFTER_ROWS = 1404;
const EXPECTED_AFTER_COMPLEXES = 1291;
...
const CHECKPOINT_PATH = resolve(DIR, "seoul-wave3-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "seoul-wave3-dryrun.json");
const COHORT_PATH = resolve(DIR, "seoul-wave3-cohort.json");
const RESULT_PATH = resolve(DIR, "seoul-wave3-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 5163 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 169 || artifact.no_published_month !== 31) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 169 || artifact.totals.would_insert !== 169 || artifact.totals.would_update !== 0) {
    failures.push("artifact totals");
  }
  const artifactSeoul = artifact.by_sido.find((row) => row.sido_code === "11");
  if (
    !artifactSeoul ||
    artifactSeoul.targets !== 169 ||
    artifactSeoul.COMPLETE !== 169 ||
    artifactSeoul.would_insert !== 169 ||
  ...
  if (records.length !== 5163) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 169 || unpublished.length !== 31) {
  ...
  if (plans.length === 169 && seoul.length !== 169) {
  ...
  if (insertedSeoul !== 169) post.push("insert counts");
