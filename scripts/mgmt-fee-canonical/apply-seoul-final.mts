/**
 * Production INSERT of the verified Seoul final COMPLETE months.
 * Rebuilds the 263 rows from the committed Seoul final checkpoint. Does not call the fee API.
 * A second --commit of the same artifact is a no-op when every stored row already matches.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-final.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-final.mts --commit
 */
...
const CHECKPOINT_SHA256 = "55fb697f0550af393c743c418091d30275b356a9ba92be56853cfdcfd127ba47";
const DRYRUN_SHA256 = "69224306dcd215c47d247a139b76fc30841feaeb7cf8b1662a805229e3b16b9d";
const COHORT_SHA256 = "e851085af7005296326fd3f36adf3e2c2d8789dc48fbe7f74fff2bb99ea589c1";
const EXPECTED_BEFORE_ROWS = 1580;
const EXPECTED_BEFORE_COMPLEXES = 1467;
const EXPECTED_AFTER_ROWS = 1843;
const EXPECTED_AFTER_COMPLEXES = 1730;
...
const DRYRUN_PATH = resolve(DIR, "seoul-final-dryrun.json");
const COHORT_PATH = resolve(DIR, "seoul-final-cohort.json");
const RESULT_PATH = resolve(DIR, "seoul-final-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (!cohort.frozen || cohort.selected !== 371 || cohort.complexes.length !== 371) failures.push("frozen cohort");
  ...
  if (artifact.api_calls !== 8214 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 263 || artifact.no_published_month !== 108) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 263 || artifact.totals.would_insert !== 263 || artifact.totals.would_update !== 0) {
    failures.push("artifact totals");
  }
  const artifactSeoul = artifact.by_sido.find((row) => row.sido_code === "11");
  if (
    !artifactSeoul ||
    artifactSeoul.targets !== 263 ||
    artifactSeoul.COMPLETE !== 263 ||
    artifactSeoul.would_insert !== 263 ||
  ...
  if (records.length !== 8214) failures.push(`checkpoint records ${records.length}`);
  ...
  if (checkpointIds.size !== 371 || cohort.complexes.some((row) => !checkpointIds.has(row.complex_id))) {
  ...
  if (plans.length !== 263 || unpublished.length !== 108) {
  ...
  if (plans.length === 263 && seoul.length !== 263) {
  ...
  if (insertedSeoul !== 263) post.push("insert counts");
