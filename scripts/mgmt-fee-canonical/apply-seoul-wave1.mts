/**
 * Production INSERT of the verified Seoul wave-1 COMPLETE months.
 * Rebuilds the 90 rows from the committed Seoul wave-1 checkpoint. Does not call the fee API.
 * A second --commit of the same artifact is a no-op when every stored row already matches.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave1.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-seoul-wave1.mts --commit
 */
...
const CHECKPOINT_SHA256 = "bbc0ab1cb850d38afb2520e0322523138195a1dabe29185c02a7f577c5633154";
const DRYRUN_SHA256 = "99af5ae56109cc541d5bfd39edc690281b2264b5c383690195e6e41418757b15";
const COHORT_SHA256 = "55094631d1ac17826083c4360933ad88d09f7c235a50714ed9d4282b57d226f8";
const EXPECTED_BEFORE_ROWS = 963;
const EXPECTED_BEFORE_COMPLEXES = 850;
const EXPECTED_AFTER_ROWS = 1053;
const EXPECTED_AFTER_COMPLEXES = 940;
...
const CHECKPOINT_PATH = resolve(DIR, "seoul-wave1-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "seoul-wave1-dryrun.json");
const COHORT_PATH = resolve(DIR, "seoul-wave1-cohort.json");
const RESULT_PATH = resolve(DIR, "seoul-wave1-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (!cohort.frozen || cohort.selected !== 100 || cohort.complexes.length !== 100) failures.push("frozen cohort");
  ...
  if (artifact.api_calls !== 501 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 90 || artifact.no_published_month !== 10) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 90 || artifact.totals.would_insert !== 90 || artifact.totals.would_update !== 0) {
    failures.push("artifact totals");
  }
  const artifactSeoul = artifact.by_sido.find((row) => row.sido_code === "11");
  if (
    !artifactSeoul ||
    artifactSeoul.targets !== 90 ||
    artifactSeoul.COMPLETE !== 90 ||
    artifactSeoul.would_insert !== 90 ||
  ...
  if (records.length !== 2730) failures.push(`checkpoint records ${records.length}`);
  ...
  if (checkpointIds.size !== 100 || cohort.complexes.some((row) => !checkpointIds.has(row.complex_id))) {
  ...
  if (plans.length !== 90 || unpublished.length !== 10) {
  ...
  if (plans.length === 90 && seoul.length !== 90) {
  ...
  if (insertedSeoul !== 90) post.push("insert counts");
