/**
 * Production INSERT of the verified Busan/Gyeongnam wave-3 COMPLETE months.
 * Rebuilds the 179 rows from the committed wave-3 checkpoint. Does not call the fee API.
 *
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave3.mts
 *   npx tsx scripts/mgmt-fee-canonical/apply-busan-gyeongnam-wave3.mts --commit
 */
...
const CHECKPOINT_SHA256 = "af35a9bdd84b22027aaba8dc8488b282c6469691f7a0418f25846224ea18af1f";
const DRYRUN_SHA256 = "b34adcf713a9f16ff5f4faa954a6c7db50279600531a00933aaba2544b90a7c7";
const EXPECTED_BEFORE_ROWS = 253;
const EXPECTED_BEFORE_COMPLEXES = 140;
const EXPECTED_AFTER_ROWS = 432;
const EXPECTED_AFTER_COMPLEXES = 319;
...
const CHECKPOINT_PATH = resolve(DIR, "busan-gyeongnam-wave3-op-checkpoint.json");
const DRYRUN_PATH = resolve(DIR, "busan-gyeongnam-wave3-dryrun.json");
const RESULT_PATH = resolve(DIR, "busan-gyeongnam-wave3-apply-result.json");
...
async function main(): Promise<void> {
  ...
  if (artifact.api_calls !== 5433 || artifact.reuse_confirmation_api_calls !== 0 || artifact.http_429 !== false) {
    failures.push("artifact api guard");
  }
  if (artifact.target_months !== 179 || artifact.no_published_month !== 21) failures.push("artifact target counts");
  if (artifact.totals.COMPLETE !== 179 || artifact.totals.would_insert !== 179 || artifact.totals.would_update !== 0) {
  ...
  if (
    !artifactBusan ||
    artifactBusan.targets !== 86 ||
    artifactBusan.COMPLETE !== 86 ||
    artifactBusan.would_insert !== 86 ||
  ...
  if (
    !artifactGyeongnam ||
    artifactGyeongnam.targets !== 93 ||
    artifactGyeongnam.COMPLETE !== 93 ||
    artifactGyeongnam.would_insert !== 93 ||
  ...
  if (records.length !== 5433) failures.push(`checkpoint records ${records.length}`);
  ...
  if (plans.length !== 179 || unpublished.length !== 21) {
  ...
  const originalHash = identityHash(before);
  const originalKeys = new Set(beforeIndex.keys());
  ...
  if (plans.length === 179 && (busan.length !== 86 || gyeongnam.length !== 93)) {
  ...
  if (collisions.length > 0) failures.push(`collisions ${collisions.length}`);
  ...
  if (insertedBusan !== 86 || insertedGyeongnam !== 93) post.push("insert counts");
