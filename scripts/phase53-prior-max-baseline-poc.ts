/**
 * Phase 5.3 — historical prior-max baseline PoC (READ-ONLY).
 *
 ...
 *
 *   npx tsx scripts/phase53-prior-max-baseline-poc.ts
 */
...
async function main() {
  ...
  for (const p of PILOTS) {
    ...
    const baselines = computeBaselines(preWarehouse, groups);
    totalBaselineRows += baselines.length;
  ...
  const out = {
    ...
    phase: "5.3-prior-max-baseline-poc",
    safety: {
  ...
  const dir = join(process.cwd(), "data/poc/phase53");
