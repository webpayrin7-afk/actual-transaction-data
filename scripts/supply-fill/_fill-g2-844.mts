...
const IDS844 = new Set<string>(JSON.parse(readFileSync("C:/dev/ziplab-wt/topology/scratch844/ids.json", "utf8")));
...
async function main() {
  ...
  const complexes = readFileSync(CADASTRAL, "utf8")
    .split("\n")
    .filter(Boolean)
    .map((line) => JSON.parse(line) as ComplexRow).filter((r) => IDS844.has(r.complexId));
  ...
  if (apply) {
    ...
    writeFileSync("data/poc/supply/g2-844-apply.json", JSON.stringify({ at: now, affected, totals }, null, 2));
    console.log(JSON.stringify({ apply: true, affected, ...totals }, null, 2));
  } else {
    writeFileSync(
      "data/poc/supply/g2-844-plan-rows.jsonl",
      statements
    ...
    writeFileSync(
      "data/poc/supply/g2-844-plan.json",
