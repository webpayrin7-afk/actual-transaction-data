"""
[지역판] build_buildings_pmtiles.py 복사본 — --region 으로 자르는 범위·메타 이름만 바꾼다 (서울 운영 빌드는 원본 파일을 쓴다).
  ...
  --region seoul(기본) | gyeonggi — REGIONS 표에 범위(경위도 여유 포함) 추가하면 다른 시도도 된다.
  바닥 면적은 건물마다 자기 위도로 계산(원본은 37.55° 고정 — 경기 남북 끝에서 ±3% 차이).
...
REGIONS = {
    # key: (서→동 경도, 남→북 위도, 메타 이름)
    "seoul": ((126.70, 127.25), (37.38, 37.75), "ziplab-seoul-buildings"),
    # 경기: 평택·안성 남단 36.89 ~ 연천 북단 38.28, 안산 풍도 126.3 ~ 가평 동단 127.85 (+여유)
    "gyeonggi": ((126.25, 127.95), (36.85, 38.35), "ziplab-gyeonggi-buildings"),
}
REGION = args[args.index("--region") + 1] if "--region" in args else "seoul"
if REGION not in REGIONS:
    raise SystemExit(f"--region {REGION}? {list(REGIONS)}")
(LON0, LON1), (LAT0, LAT1), META_NAME = REGIONS[REGION]
...
def shp_paths(root: Path, tmp: Path) -> list[Path]:
    # 큰 시도(경기 등)는 zip 안이 SHP 100만 건씩 여러 개로 나뉘어 있다: X.shp, X(2).shp, X(3).shp … → 모두 읽는다
...
# 지역(+여유) 범위, 정규 좌표 [minX, minY, maxX, maxY] — 위도는 북쪽이 Y 작음
_a = merc_norm(LON0, LAT1)
_b = merc_norm(LON1, LAT0)
...
apts: list[int] = []
try:
    shps = shp_paths(SRC, tmpdir)
    readers = [shapefile.Reader(str(p.with_suffix("")), encoding="cp949") for p in shps]
    for r in readers:
        names = [f[0] for f in r.fields[1:]]
        for need in ("A8", "A16", "A26"):
            if need not in names:
                raise SystemExit(f"필드 {need} 없음: {names}")
    log(f"read {[p.name for p in shps]}: {[len(r) for r in readers]} records")
    bad = 0
    outside = 0
    all_records = itertools.chain.from_iterable(r.iterShapeRecords(fields=["A8", "A16", "A26"]) for r in readers)
    for n, sr in enumerate(all_records):
...
if SPBD:
    ...
    hit = (ratio >= OVERLAP) & (inter >= OVERLAP * s_area)  # SPBD 쪽에서도 30% 이상 = 같은 건물
    SA[si[hit & (A[ai] == 1)]] = 1
    keep = ~drop
    # 점검용: 뺀 AL_D010 중 10층(33m) 이상인데 겹치는 SPBD가 모두 80% 미만 높이인 것 (낮아지는 회귀)
    best = np.zeros(len(geoms), dtype=np.int32)
    np.maximum.at(best, ai[ratio >= OVERLAP], np.array(s_h, dtype=np.int32)[si[ratio >= OVERLAP]])
    tall_lower = int((drop & (H >= 33) & (best < 0.8 * H)).sum())
    merge_stats = {
        ...
        "tall_dropped_lower": tall_lower,
        "spbd_residential": int(SA.sum()),
    }
    log(f"SPBD merge: {merge_stats}")
    geoms = np.concatenate([geoms[keep], S])
    H = np.concatenate([H[keep], np.array(s_h, dtype=np.int32)])
    A = np.concatenate([A[keep], SA])
# 바닥 면적(대략, m²): 정규 좌표 면적 × (지구둘레)² × cos²(위도) — 위도는 건물마다(바운딩 박스 가운데)
_bb = shapely.bounds(geoms)
_lat = np.arctan(np.sinh(np.pi * (1 - (_bb[:, 1] + _bb[:, 3])))) # Y중심 = (minY+maxY)/2 → 1-2Y
AREA_M2 = shapely.area(geoms) * (2 * HALF) ** 2 * np.cos(_lat) ** 2
del _bb, _lat
del polys
