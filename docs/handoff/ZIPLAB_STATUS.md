...
기준 시각: 2026-09-24 01:46 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **11,881/24,243**, ins=2,438,799, fail=48 · chungnam SALE 201702 | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` / PR #136 | **인천 APPLIED** (+1,015). fee_table 3,938/3,825. **부산 ACQUIRING** 176/993 (~17%) COMPLETE 161 + NO_PUB 15, api~4,970, seg 4→. 대구 이하 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | 502 — 대기 (마지막 01:25Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (01:46Z Production)
