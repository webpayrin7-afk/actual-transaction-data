...
기준 시각: 2026-09-24 11:28 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **17,230/24,243**, ins=3,268,397, fail=90 · gyeongbuk SALE 202205 | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` / PR #136 | **인천 APPLIED** (+1,015). **부산 APPLIED** dry-run PASS → insert **881** (verify IDEMPOTENT_NOOP). fee_table **4,819행/4,706단지** (applied_total 1,896). **대구 ACQUIRING** cohort 1,151, api~170, n=2. 광주·전남 이하 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
- 관리비 **4,819행/4,706단지** (인천+부산 적용 완료). 대구 acquire 진행 중. 전국 거래 백필 진행 중.

## 3. 오너 결정 대기

1. 좌표 잔여 ~3,612(exact-PNU): 로컬 AL_D002에서 `pnu_not_found` 다수(부산·대구). 최신 시·도 지적도 zip 필요 여부 조사 후 요청 예정.
2. 관리비: 인천·**부산 적용 완료**. **대구 acquire 자동 시작**(cohort 1,151). 경남 등은 이후 PENDING.
