...
기준 시각: 2026-09-24 05:18 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **13,750/24,243**, ins=2,809,362, fail=69 · daejeon SALE 201906 (대전 초반 정체·fetch fail↑, heartbeat 생존) | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` / PR #136 | **인천 APPLIED** (+1,015). fee_table 3,938/3,825. **부산 ACQUIRING** 488/993 (~49%) COMPLETE 410 + NO_PUB 78, api~12,650, seg 12→. 대구 이하 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | disconnect — 대기 (마지막 05:07Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (05:18Z Production)
...
- 관리비 **3,938행/3,825단지** (인천 +1,015 적용 완료). 부산 acquire 진행 중. 전국 거래 백필 진행 중.

## 3. 오너 결정 대기

1. 좌표 잔여 ~3,612(exact-PNU): 로컬 AL_D002에서 `pnu_not_found` 다수(부산·대구). 최신 시·도 지적도 zip 필요 여부 조사 후 요청 예정.
2. 관리비 부산·경남: 인천 적용 완료. **부산 acquire 자동 시작**(cohort 993). 경남은 이후 PENDING.
