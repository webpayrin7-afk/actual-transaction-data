...
기준 시각: 2026-09-23 13:44 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 작업 | 브랜치 / PR | 상태 (13:44Z) | 재개 |
|---|---|---|---|
| 전월세 과거 이력 2011-01~2022-09 (서울 25구) | tmux `rent-backfill` /workspace | **3,225/3,525** 셀, 약 166.1만 행, 실패 0 (실행 중, 남은 ~300셀) | 완료 후 `region-jeonse` 시리즈 확장 → materialize → test |
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **2,700/24,243** 셀, 66.8만 행, fail=4(retry queue, 일시 fetch failed 2건 포함) · 충남 RENT 진행 중 | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` | **인천 ACQUIRING** COMPLETE 373 + NO_PUB 27 / 1,093 (~37%), api~11,370, seg 10→11, 429 0. 부산 이하 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | 미국 AWS IP 502/disconnect — 대기 (tmux `vworld-watch`, 마지막 13:21Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (13:44Z Production)
