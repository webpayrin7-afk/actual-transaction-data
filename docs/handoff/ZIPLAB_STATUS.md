...
기준 시각: 2026-09-23 18:14 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **5,583/24,243**, ins=1,265,529, fail=0 · gyeongbuk RENT — **18:14Z stall 재개**(반복 stall ~15–17분) | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` | **인천 ACQUIRING** COMPLETE 634 + NO_PUB 49 / 1,093 (~62%), api~19,220, seg 17, unfinished~410. 부산 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | disconnect/502 — 대기 (마지막 17:43Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (18:14Z Production)
