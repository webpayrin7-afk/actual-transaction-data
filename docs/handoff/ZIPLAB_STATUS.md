...
기준 시각: 2026-09-24 14:22 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **19,141/24,243**, ins=3,359,446, fail=98 · gyeongnam SALE 201909 · hb 신선·PID 365030 정상 | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` / PR #136 | **인천 APPLIED** (+1,015). **부산 APPLIED** (+881). fee_table **4,819/4,706**. **대구 ACQUIRING** cohort 1,151 · terminal **217**/1151 (COMPLETE 205 / NO_PUB 12) · api~6,270 · seg5 · ~35 api/min · timeout=8. 광주·전남 이하 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | WAIT RemoteDisconnected (마지막 14:08Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (14:22Z Production)
...
- 관리비 **4,819행/4,706단지** (인천+부산 적용 완료). 대구 acquire ~18.9% terminal (217/1151). 전국 거래 백필 진행 중.

## 3. 오너 결정 대기

1. 좌표 잔여 ~3,612(exact-PNU): 로컬 AL_D002에서 `pnu_not_found` 다수(부산·대구). 최신 시·도 지적도 zip 필요 여부 조사 후 요청 예정.
2. 관리비: 인천·**부산 적용 완료**. **대구 acquire ~18.9%**(217/1151, api~6,270). 경남 등은 이후 PENDING.
