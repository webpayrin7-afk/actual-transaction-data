...
기준 시각: 2026-09-23 16:06 UTC · 작성: Cursor 데이터 감시(Auto/Composer) · living `cursor/vworld-watch-6779`
...
| 작업 | 브랜치 / PR | 상태 (16:06Z) | 재개 |
|---|---|---|---|
| 전월세 과거 이력 2011-01~2022-09 (서울 25구) | tmux `rent-backfill` /workspace | **완료** jobs=3,525 written=3,515 ins=1,976,686 fail=10(관악·송파·강동 일부 월 fetch/timeout). 재시도 후보 10키 | 실패 10셀 재시도 여지 |
...
| 매매·전월세 전국 전체 이력 missing-only | `cursor/tx-registration-status-1922` / PR #131 | **3,573/24,243**, 93.7만 행, fail=12 · daejeon RENT | `cd /home/ubuntu/wt/tx && bash scripts/full-history/start.sh` |
| 관리비 시·도 확장 | `cursor/mgmt-fee-provinces-6779` | **인천 ACQUIRING** COMPLETE 516 + NO_PUB 36 / 1,093 (~50%), api~15,620, seg 13. 부산 PENDING | `cd /home/ubuntu/wt/fee && bash scripts/mgmt-fee-canonical/start-provinces.sh` |
...
| VWorld 감시 | `cursor/vworld-watch-6779` / PR #135 | 502/disconnect — 대기 (마지막 15:49Z) | `bash scripts/living/vworld-watch.sh` |
| 건축물 동·세대 | PR #115 | **완료** | — |
| 단지 기본정보(HERO) | PR #122 | 원 에이전트 쪽에서 계속 | `bash scripts/profile-national-background-start.sh` |
| 주변 공급(청약홈) | PR #124 | 완료 | — |

주요 수치 (16:06Z Production)
